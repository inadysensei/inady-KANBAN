...
- **Sessions**: for cursor/claude the conversation id is created before launch, so a card always has something to reattach to. New run: `cursor-agent --resume <id> "<prompt>"` / `claude --session-id <id> --permission-mode auto "<prompt>"`; resume drops the prompt (claude keeps `--permission-mode auto`). cline mints its own id (its `--id` is resume-only), so it launches without one — `cline -i --auto-approve true -P cline-pass "<prompt>"` — and the board recovers the id from `cline history` for resume (`cline -i … --id <id>`). On an untrusted folder cursor's/claude's one-time trust prompt is auto-accepted (it's your own repo); cline auto-approves so it has none.
