...
- The conversation UUID is **pre-issued** before the PTY launches (so `agent_sessions.agentSessionId` exists first):
  ...
  In both, the initial prompt is a **positional arg** the TUI auto-submits (no stdin pre-feed; survives a trust dialog). **cursor** keeps per-command approval (no `--force` / bypass flags); **claude** launches with `--permission-mode auto` on both initial launch and resume (it's a per-run setting, not pinned to the conversation like model/effort, so a resumed session launches the same way).
- **Model selection differs by CLI.** claude takes **separate** `--model`/`--effort`, pinned to the conversation (passed only at initial launch). cursor takes a **single combined** `--model <id>` (effort baked in — there's no cursor `--effort`); `create-chat` takes no model, so `--model` is **per-invocation** and is re-passed on **every** cursor launch incl. resume (else resume reverts to cursor's default). cursor validates the id at runtime.
